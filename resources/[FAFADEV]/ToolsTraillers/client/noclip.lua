...
function SetNoclipState(state)
    ...
    if state then
        ...
        local playerRot = GetGameplayCamRot()
        
        -- Vérifier que le freecam est disponible
        if not SetEnabled or not SetPosition or not SetRotation then
            lib.notify({
                title = 'ToolsTraillers',
                description = 'Erreur: Système freecam non disponible',
                type = 'error'
            })
            return
        end
        ...
        SetRotation(playerRot.x, playerRot.y, playerRot.z)
        
        -- Rendre le joueur invisible et invincible
        ...
        -- Créer un thread pour maintenir la position du joueur
        CreateThread(function()
            while isNoclipActive and IsEnabled() do
                local camPos = GetPosition()
                SetEntityCoords(ped, camPos[1], camPos[2], camPos[3], false, false, false, false)
                
                local vehicle = GetVehiclePedIsIn(ped, false)
                if vehicle ~= 0 then
                    SetEntityCoords(vehicle, camPos[1], camPos[2], camPos[3], false, false, false, false)
                end
        ...
        lib.notify({
            title = 'ToolsTraillers',
            description = 'No-clip activé',
            type = 'success'
        })
    else
        -- Désactiver le freecam
        SetEnabled(false)
...
function ToggleNoclip()
    SetNoclipState(not isNoclipActive)
...
RegisterNetEvent('toolstraillers:toggleNoclip', function()
    ToggleNoclip()
end)
...
CreateThread(function()
    while true do
        if isNoclipActive and IsEnabled and IsEnabled() then
            -- Position actuelle
            local pos = GetPosition()
            local posText = string.format("~b~Position: ~w~%.1f, %.1f, %.1f", pos[1], pos[2], pos[3])
            ...
            SetTextOutline()
            SetTextEntry("STRING")
            AddTextComponentString("~y~NO-CLIP ACTIF~s~\n~w~WASD: Déplacement~s~\n~w~Q/E: Monter/Descendre~s~\n~w~Shift: Vitesse rapide~s~\n~w~Alt: Vitesse lente~s~\n" .. posText)
